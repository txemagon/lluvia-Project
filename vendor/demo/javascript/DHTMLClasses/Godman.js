...
<<<<<<< HEAD
    function initialize() {
        ...
        that.state.running.run.steady = God.prototype.run
...
God.prototype.run = function() {
    ...
    if (!this.owner.movieOrigin) {
        this.owner.movieOrigin = arguments[0]
        this.owner.lastCycle = this.owner.movieOrigin
    }

    for (var i = 0; i < this.owner.human.length; i++)
        this.owner.human[i].run(arguments[0])
    this.owner.lastCycle = arguments[0]
...
Human.prototype.setx = function(x) {
    this.X = x;
    this.panel.style.left = Math.ceil(x) + "px";
}
Human.prototype.sety = function(y) {
    this.Y = y;
    this.panel.style.top = Math.ceil(y) + "px";
}
Human.prototype.x = function() {
    return this.X;
}
Human.prototype.y = function() {
    return this.Y;
}
